<!--
markup.xsl - an XSLT stylesheet to markup words and phrases in text Copyright (C) 2000 Dr Jeni Tennison This program is free software; 
                                    ...
                                    <xsl:choose>
                                          <xsl:when test="string($match)">
                                                <xsl:variable name="first">
                                                      <xsl:if test="contains($punctuation, substring($match, 1, 1))">
                                                            <xsl:value-of select="substring($match, 1, 1)"/>
                                                      </xsl:if>
                                                </xsl:variable>
                                                <xsl:variable name="last">
                                                      <xsl:if test="contains($punctuation, substring($match, string-length($match)))">
                                                            <xsl:value-of select="substring($match, string-length($match))"/>
                                                      </xsl:if>
                                                </xsl:variable>
