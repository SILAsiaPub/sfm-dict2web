 ...
 -->
      ...
      <xsl:template match="/*">
           ...
           --&gt;
                              ...
                              <xsl:for-each-group select="record" group-by="substring(cite:lower-remove-accents(ie),1,1)">
                                    ...
                                    <xsl:element name="alpha">
                                          <xsl:attribute name="key">
                                                <xsl:choose>
                                                      <xsl:when test="substring(cite:lower-remove-accents-word(ie),1,1) = 'ng;'">
                                                            <xsl:text>ng</xsl:text>
                                                      </xsl:when>
                                                      <xsl:otherwise>
                                                            <xsl:value-of select="substring(cite:lower-remove-accents(ie),1,1)"/>
                                                      </xsl:otherwise>
                                                </xsl:choose>
                                                <!-- <xsl:sequence select="substring(cite:lower-remove-accents(ie),1,1)" /> -->
                                          </xsl:attribute>
                                          <xsl:apply-templates select="current-group()">
                                                <xsl:sort collation="http://saxon.sf.net/collation?rules={encode-for-uri($customcollation)}" select="cite:lower-remove-accents-word(cite:word-no-number(ie))"/>
          ...
          --&gt;
                              ...
                              <xsl:for-each-group select="record" group-by="substring(cite:lower-remove-accents(ie),1,1)">
                                    <xsl:sort select="substring(cite:lower-remove-accents(ie),1,1)"/>
                                    <xsl:element name="alpha">
                                          <xsl:attribute name="key">
                                                <xsl:sequence select="substring(cite:lower-remove-accents(ie),1,1)"/>
                                          </xsl:attribute>
                                          <xsl:apply-templates select="current-group()">
                                                <xsl:sort collation="{$default-collation}" select="cite:lower-remove-accents(ie)"/>
                                                <xsl:sort collation="{$default-collation}" select="cite:lower-remove-accents(lx)"/>
                                          </xsl:apply-templates>
                                    </xsl:element>
                              </xsl:for-each-group>
                        </xsl:otherwise>
